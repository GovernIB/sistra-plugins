...
public class SeyconMockDatabaseLoginModule extends DatabaseServerLoginModule {
	...
	protected Principal createIdentity(String username) throws Exception {
		...
		if (username != null && username.equals( (String) options.get("unauthenticatedIdentity"))){
			log.debug ("Creating SeyconPrincipal para "+username);
			return new SeyconPrincipal();
		}
		...
		{
			log.debug ("Creating SeyconPrincipal para "+username);
			return new SeyconPrincipal(username, ui.fullName, ui.nif);
		}
	 ...
	 protected Group[] getRoleSets() throws LoginException
	   {
	      ...
	      if ( ! (principal instanceof SeyconPrincipal) )
	      {
	      	if (log.isTraceEnabled()) log.trace("Principal "+principal+" not a SeyconPrincipal");
	      	return new Group[0];
	      }
