...
public class Configuracion {
	...
	private void readPropertiesFromSAR() throws Exception {
		...
		for (Iterator it = propSystem.keySet().iterator(); it.hasNext();) {
			...
			if (key.startsWith(PREFIX_SAR_PLUGINS + "plugin-smsbit")) {
				props.put(key.substring((PREFIX_SAR_PLUGINS + "plugin-smsbit").length() + 1), value);
			}			
		}
	}
	...
	private void readPropertiesFromFilesystem() throws Exception {
		props = new Properties();
		props.load(new FileInputStream(System.getProperty("ad.path.properties") + "sistra/plugins/plugin-smsbit.properties"));
	}
