...
public class JustificantePago extends DocXml{
	...
	private void initDatosPago(Node nodo)
	{
		...
		if (l_child != null)
		{
			...
			{	
				...
				//Node l_xchild = UtilXml.getFirstChild(l_child);
				do 
				{
			...
			{
				log.error(" Error obteniendo información del nodo DATOS_PAGO. ", ex);		
			}
		}
	}
	...
	private int initDatos(Node nodo)
	{
		...
		if (l_child != null)
		{
			...
			{
				log.error(" Error obteniendo información del nodo JUSTIFICANTE_PAGO. " , ex);		
				return -1;
			}
		}
		return 0;
	...
	public void initialize(){
		...
		try{		
			...
			while (l_nodo != null && l_nodo.getNodeType() != Document.ELEMENT_NODE && 
					   !(l_nodo.getNodeName().equalsIgnoreCase("JUSTIFICANTE_PAGO")))
			{
