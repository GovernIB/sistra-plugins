...
public class UtilsRegweb3 {
	...
	public static RegWebAsientoRegistralWs getAsientoRegistralService(String entidad) throws Exception  {

		final String endpoint = ConfiguracionRegweb3.getInstance().getProperty("regweb3.endpoint.asiento");
		String user = obtenerUsuarioEntidad(entidad);
		String pass = obtenerPasswordEntidad(entidad);

		// Url WSDL: local o remoto segun haya proxy
		URL wsdl = obtenerUrlWsdl(endpoint, "RegWebAsientoRegistral");
	...
	public static Long obtenerIdiomaRegistro(String idioma) {
		Long res = ConstantesRegweb3.IDIOMA_OTROS;
		if ("es".equals(idioma)) {
			res = ConstantesRegweb3.IDIOMA_CASTELLANO;
		} else if ("ca".equals(idioma)) {
			res = ConstantesRegweb3.IDIOMA_CATALAN;
		} else if ("en".equals(idioma)) {
			res = ConstantesRegweb3.IDIOMA_INGLES;
		}
		return res;
	}
